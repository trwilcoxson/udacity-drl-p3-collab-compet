...
class UnityEnvironment(object):
    ...
    def step(self,  vector_action=None, memory=None, text_action=None) -> AllBrainInfo:
        ...
        if self._loaded and not self._global_done and self._global_done is not None:
            if isinstance(vector_action, (int, np.int64, float, np.float64, list, np.ndarray)):
            ...
            if isinstance(memory, (int, np.int64, float, np.float64, list, np.ndarray)):
                if self._num_external_brains == 1:
    ...
    def _flatten(arr):
        ...
        """
        if isinstance(arr, (int, np.int64, float, np.float64)):
            arr = [float(arr)]
